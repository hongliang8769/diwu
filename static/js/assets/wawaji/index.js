...
var $appointmentWrapper = util.getById('appointment-wrapper'); // 1.预约和取消预约按钮
																// 以及 2.其下排队信息
																// 的盒子
...
var $beforeQueueNum = util.getById('before-queueNum'); // 预约之前的显示块 当前排队人数 xxx 人
														// 此处为xxx的人数数值
var $afterQueue = util.getById('after-queue'); // 预约成功之后的显示块 你已预约成功，当前排在第 xxx 位
var $afterQueueNum = util.getById('after-queueNum'); // 预约成功之后的显示块
														// 你已预约成功，当前排在第 xxx
														// 位，此处为xxx位数值
...
var $operateWrapper = util.getById('operate-wrapper'); // 操作元素
														// 1.上下左右按钮、2.抓取按钮的盒子
...
if (window.location.protocol === 'https:') {
    server = 'wss://wsliveroom' + appid + '-api.zego.im:8282/ws'; // wawaji接入服务器地址
																	// ---
																	// 即构下发的server地址
    logUrl = ''; // log服务器地址 --- 可填可不填
    loginTokenUrl = 'https://wsliveroom' + appid + '-api.zego.im:8282/token'; // 登录token派发地址
																				// ---
																				// 业务后台自己的地址
    payTokenUrl = 'https://wsliveroom' + appid + '-api.zego.im:8282/pay'; // 支付地址
																			// ---
																			// 业务后台自己的地址
} else {
    server = 'ws://test2-wsliveroom-api.zego.im:8181/ws'; // wawaji接入服务器地址 ---
															// 即构下发的server地址
    logUrl = ''; // log服务器地址 --- 可填可不填
    loginTokenUrl = 'http://my.newsmy-car.com/index.php?g=Home&m=Diwu&a=getToken'; // 登录token派发地址
																					// ---
																					// 业务后台自己的地址
    payTokenUrl = 'http://my.newsmy-car.com/index.php?g=Home&m=Diwu&a=pay'; // 支付地址
																			// ---
																			// 业务后台自己的地址
}
...
window.onload = function () {
        ...
        if (useStreamList.length !== 0) {
            ...
            for (var m = 0; m < useStreamList.length; m++) {
                ...
                if (!playFlag) {
					 ...
					 */
                    // zg.startPlayingStream(useStreamList[m].stream_id,
					// useStreamList[m].videoView);
    ...
    $switchBtn.addEventListener('click', function (e) {
        ...
        // zg.sendRoomMsg(2, 1, 'sdfsdf', function(seq, msdid, msgcategory,
		// msgtype, msgcontent){
    ...
    /*
	 * function updateStreamDecode(frontDecode) { for (var i = 0; i <
	 * useLocalStreamList.length; i++) { if
	 * (useLocalStreamList[i].stream_id.endsWith("_2")) {
	 * zg.enableDecode(useLocalStreamList[i].stream_id, !frontDecode); } else {
