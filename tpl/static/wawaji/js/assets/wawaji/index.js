...
//xmlhttp2.open("GET", '{pigcms::STATICS}/wawaji/js/jZego/jsmpeg-stub.min.js', false);
//xmlhttp2.send();
// dom元素
...
var $upornotCancel = util.getById('upornot-cancel'); // 放弃上机按钮
...
var loginTokenUrl = '';
var payTokenUrl = '';
var siteUrl='http://my.newsmy-car.com/';
if (window.location.protocol === 'https:') {
    ...
    logUrl = ''; //log服务器地址          --- 可填可不填
    loginTokenUrl = siteUrl+'index.php?g=Home&m=Wawaji&a=getToken'; //登录token派发地址       --- 业务后台自己的地址
    payTokenUrl =  siteUrl+'index.php?g=Home&m=Wawaji&a=pay'; //支付地址               --- 业务后台自己的地址
}
...
//获取用户id
var wecha_id =util.getUrlParam('wecha_id');
...
if(!wecha_id){
	...
	idName = wecha_id;
	util.setLocal('idName', wecha_id);
}
nickName = "u" + idName;

//获取房间id
roomID =util.getUrlParam('roomid');
...
window.onload = function () {
    ...
    */
    ...
    var waitPosition = 0;
    var RECVCMD = {
        broadcast: 257,        //7 游戏信息更新全员广播，游戏信息更新	    257	Server—>Client
        appointment: 272,      //1 预约  预约上机回复（内含预约上机结果）	272	Server—>Client
        cancelAppointment: 274,//2 取消预约  取消预约回复	            274 Server—>Client
        upSelect: 258,         //3 上机  准备上机	                258	Server—>Client
        upSelectRsp: 273,      //4 确认上机 回复收到上机选择	        273	Server—>Client	
        operateResult: 260,    //6 结果反馈  游戏结果	            260	Server—>Client
        gameInfo: 275          //7 游戏信息更新  回复获取游戏信息	        275	Server—>Client
    };

    //from_userid	服务器用户唯一id	字符串
    //from_idname	用户id	字符串
    //custom_content	错误信息	字符串
    // 接收服务端主动推过来消息的接口
    zg.onRecvCustomCommand = function (from_userid, from_idName, custom_content) {
        ...
        if (custom_content.cmd === RECVCMD.broadcast) {
            ...
            // 告知本次预约请求是否成功，预约成功则进入排队阶段，否则失败(预约上机回复（内含预约上机结果）)
    ...
    function appointmentHandler(custom_content) {
        if (custom_content.data.result == 1) {
            ...
            util.setLocal('sessionID', sessionID);//预约成功后，才从这里获取sessionID，是即构服务器回调传过来的
        }
    ...
    function pay() {
        ...
        var xmlhttp;var payTokenUrlOpen;
        xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4) {
                if (xmlhttp.status == 200) {
                    ...
                    console.log('payTokenUrl = ', payTokenUrlOpen);
                    upToPlayHandler();
                } else {
                    alert("请求支付失败");
                }
        ...
        payTokenUrlOpen=payTokenUrl + "&app_id=" + appid + "&id_name=" + idName + "&session_id=" + sessionID + "&confirm=1" + "&time_stamp=" + payTimestamp + "&item_type=" + itemType + "&item_price=" + itemPrice
        //实际情况，使用客户端自己的域名地址获得payToken
        xmlhttp.open("GET", payTokenUrlOpen, true);
        xmlhttp.send();
    }
    ...
    function upToPlay() {
        ...
        console.log('确定上机');
        console.log('payTimestamp = ', payTimestamp);
        operateStatus = true;
    ...
    function upSelectRspHandler(custom_content) {
        ...
        if (resultCode === 1) {
            ...
            console.log('payToken = ', payToken);
            clearInterval(upToPlayTimer);//如果主动发送的取消上机，没有config加密数据部分，就不会返回这个错误。
            return;
    ...
    function operateResultHandler(custom_content) {
        ...
        if (custom_content.data.result === 1) {//通过解密内容进行判断
            // alert('恭喜您抓取到娃娃了！');
