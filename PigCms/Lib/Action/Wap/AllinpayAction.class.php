...
class AllinpayAction extends BaseAction{
	...
	public function pay(){
		...
		if(empty($_GET['platform'])){
			$return_url = C('site_url').'/index.php?g=Wap&m=Allinpay&a=r_u&wx_id='.$now_wxuser['wx_id'].'&we_id='.$now_wecha_user['we_id'].'&from='.$_GET['from'].'&cardid='.$cardid;
		}else{
			$return_url = C('site_url').'/index.php?g=Wap&m=Allinpay&a=r_u&wx_id='.$now_wxuser['wx_id'].'&we_id='.$now_wecha_user['we_id'].'&from='.$_GET['from'].'&pl=1&cardid='.$cardid;
		}
	...
	public function r_u(){
		...
		if(!$verify_result['error']){
			...
			$this->redirect('/index.php?g=Wap&m='.$from.'&a=payReturn&token='.$orderInfo['token'].'&wecha_id='.$orderInfo['wecha_id'].'&orderid='.$verify_result['order_id'].'&cardid='.$cardid);
		}else{
