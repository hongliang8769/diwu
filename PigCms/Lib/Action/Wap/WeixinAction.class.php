...
class WeixinAction extends BaseAction{
	...
	public function new_pay(){
		...
		if (!isset($_GET['code'])){
			//触发微信返回code码
			$url = $jsApi->createOauthUrlForCode(urlencode($this->siteUrl.'/wxpay/index.php?g=Wap&m=Weixin&a=new_pay&price='.$_GET['price'].'&orderName='.urlencode($_GET['orderName']).'&single_orderid='.$_GET['single_orderid'].'&showwxpaytitle=1&from='.$_GET['from'].'&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&cardid='.$cardid.'&redirect='.$redirect));
		...
		$returnUrl = $this->siteUrl.'/index.php?g=Wap&m='.$from.'&a=payReturn&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&orderid='.$orderid.'&cardid='.$cardid;
		$redirect = $this->_request('redirect');
		...
		if (!empty($redirect)){
			...
			$returnUrl =U($urlinfo[0],$parms);
			$returnUrl =str_ireplace("/wxpay","",$returnUrl);
		}
		...
		$unifiedOrder->setParameter("attach",'token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&from='.$_GET['from'].'&cardid='.$cardid);//附加数据
	...
	public function pay(){
		...
		$wxPayHelper->setParameter("notify_url", $this->siteUrl.'/index.php?g=Wap&m=Weixin&a=return_url&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&from='.$_GET['from'].'&cardid='.$cardid);
		...
		$returnUrl='/index.php?g=Wap&m='.$from.'&a=payReturn&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&orderid='.$orderid.'&cardid='.$cardid;
		$this->assign('returnUrl',$returnUrl);
