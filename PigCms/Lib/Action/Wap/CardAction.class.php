...
class CardAction extends WapAction{
	...
	public function __construct(){
		...
		if (!$this->wecha_id){//wecha_id不是查询Userinfo表后的结果进行的判断。
		...
		$thisMember=$member_card_create_db->where(array('token'=>$this->token,'wecha_id'=>$this->wecha_id))->find();
		if ($thisMember&&$this->cardid<1){
			$this->cardid=$thisMember['cardid'];
	...
	public function index(){
		...
		M('Member_card_coupon')->where('token=\''.$this->token.'\' AND cardid<2')->save(array('cardid'=>$cardByToken['id']));//优惠劵活动
		M('Member_card_vip')->where('token=\''.$this->token.'\' AND cardid<2')->save(array('cardid'=>$cardByToken['id']));
		M('Member_card_integral')->where('token=\''.$this->token.'\' AND cardid<2')->save(array('cardid'=>$cardByToken['id']));//礼品卷活动
		//transfer end
	...
	function cardadmin(){
		...
		if($this->isadmin['level']>0){
			$userinfo_db=M('Userinfo');
    		$totaluser=$userinfo_db->where(array('token'=>$this->token))->count();
			$this->assign('totaluser',$totaluser);
			
			$member_card_create_db=M('Member_card_create');
    		$totalMember=$member_card_create_db->where(array('token'=>$this->token,'wecha_id'=>array('neq','')))->count();
			...
			$totalpay = $record->where(array('token'=>$this->token,'paytype'=>'CardPay','paytime'=>array('between','$day_start,$day_end'),'paid'=>1))->sum('price');
			$totalchong = $record->where(array('token'=>$this->token,'paytype'=>'weixin','paytime'=>array('between','$day_start,$day_end'),'paid'=>1))->sum('price');
			...
			if(!empty($search)){
				$error=0;
				/*if (preg_match_all("/^([\x81-\xfe][\x40-\xfe])+$/", $search, $match)) {
					//echo '全部是汉字';  
					if(self::abslength($search)<=1)  $error=-6;
				} else {
					//echo '不全是汉字';
					if(self::abslength($search)<=2)  $error=-7;
				}*/
				if (preg_match("/([\x81-\xfe][\x40-\xfe])/", $search, $match)) {
					//echo '含有汉字'; 
					if(self::abslength($search)<=1)  {$error=-6;}   
				}else{
					//echo '不含有汉字';
					if(self::abslength($search)<=1)  {$error=-7;}
				}
				if($error==-6||$error==-7){
				...
				$map['tp_Userinfo.tel']  = array('like', '%'.$search.'%');
				$map['tp_Userinfo.truename']  = array('like','%'.$search.'%');
				$map['tp_Userinfo.beizhu']  = array('like','%'.$search.'%');
				$map['tp_Usercarinfo.carnumber']  = array('like','%'.$search.'%');
				$map['_logic'] = 'or';
				$where['_complex'] = $map;
			}
		$Usernum 	= M('Userinfo')->join('tp_Usercarinfo on tp_Usercarinfo.wecha_id=tp_Userinfo.wecha_id')->join('tp_Member_card_create on tp_Member_card_create.wecha_id=tp_Userinfo.wecha_id')->where($where)->count();
			//var_dump(M('Userinfo')->getLastSql());exit();		

		$Page       = new Page($Usernum,15);
		if(!empty($search)){
				$Page->parameter   .=   'token='.$this->token.'&wecha_id='.$this->wecha_id.'&search='.$search;
		}		
//			$show       = $Page->show();
		$allcard = M('Userinfo')->join('tp_Usercarinfo on tp_Usercarinfo.wecha_id=tp_Userinfo.wecha_id')->join('tp_Member_card_create on tp_Member_card_create.wecha_id=tp_Userinfo.wecha_id')->field('tp_Userinfo.*,tp_Userinfo.id as uid,tp_Usercarinfo.carnumber,tp_Usercarinfo.cartype,tp_Usercarinfo.id as cid,tp_Member_card_create.cardid,tp_Member_card_create.id as itemid,tp_Member_card_create.number')->where($where)->order('tp_Userinfo.getcardtime desc')->limit($Page->firstRow.','.$Page->listRows)->select();
		//var_dump(M('Userinfo')->getLastSql());exit();
	//			var_dump($allcard);exit();
				
			foreach($allcard as $key=>$value){
				$thisCardinfo=M('Member_card_set')->where(array('token'=>$this->token,'id'=>$value['cardid']))->find();
				$thisCardinfo?$allcard[$key]['cardname'] = $thisCardinfo['cardname']:'非会员';	
				if($value['cardid']){
					$allcard[$key]['cardimg']=$thisCardinfo['logo'];
				}else{
					$allcard[$key]['cardimg']='/tpl/Wap/default/common/card/images/power12.png';
				}
			}
    ...
    function card(){
    	...
    	$thisMember=$member_card_create_db->where(array('token'=>$this->token,'wecha_id'=>$this->wecha_id))->find();
		if (!$thisMember){
			$cardInfo=M('member_card_set')->where(array('token'=>$this->token,'miniscore'=>0,'minimomey'=>0))->find();
			if(!$cardInfo)
			{
				$error=-5;//商家还未设置普通会员卡
			}else{
				$error=-4;
				header('Location:/index.php?g=Wap&m=Userinfo&a=index&token='.$this->token.'&wecha_id='.$this->wecha_id.'&cardid='.$cardInfo['id']);
			}
		}else{
			$userinfo_db=M('Userinfo');
    		$userInfos=$userinfo_db->where(array('token'=>$this->token,'wecha_id'=>$this->wecha_id))->select();
    		...
    		if ($userInfos){
    			$userScore=intval($userInfos[0]['total_score']);
				$balance=intval($userInfos[0]['balance']);
    			$userInfo=$userInfos[0];
    		}
    		$this->assign('userScore',$userScore);
			$this->assign('balance',$balance);
			
			$cardid=$thisMember['cardid'];
    ...
    public function integral(){
       ...
       public function myself_coupon(){
    	...
    	if($type  == 3){
			...
			$data 	= M('dish_set_record')->where($cwhere)->select();
			foreach ($data as $k=>$n){
				$data[$k]['get_count'] 	= 1;
				$data[$k]['count'] 	= 1;
    	...
    	$thisCard=M('Member_card_set')->where(array('token'=>$this->token,'id'=>intval($this->cardid)))->find();
		$this->assign('thisCard',$thisCard);
    	$this->assign('firstItemID',$data[0]['id']);
    	$this->assign('list',$data);
    	$this->assign('type',$type);
    ...
    public function my_coupon(){
    	...
    	if($type  == 3){
			...
			$where 	= array('token'=>$this->token,'isopen'=>1);
    		/*if($type == 1){
    			$where['type'] = 1;
    		}else if($type == 2){
    			$where['type'] = 0;
    		}*/	
    		$data	= M('Dish_set')->where($where)->order('sort asc')->select();
			//var_dump(M('Member_card_coupon')->getlastsql());exit();
    		foreach ($data as $k=>$n){
    			$data[$k]['info']	 	= unserialize($n['info']);
				$data[$k]['orderid'] 	= date('YmdHis',time()).mt_rand(1000,9999);
				//$cwhere = array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'cardid'=>$thisCard['id'],'coupon_type'=>$type,'coupon_id'=>$n['id']);
    			$cwhere = array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'coupon_id'=>$n['id']);
    			$dish_set_recorddb 	= M('dish_set_record')->where($cwhere)->find();
				if(!empty($dish_set_recorddb)){
					$data[$k]['get_count'] 	= 1;
    				$data[$k]['count'] 	= $n['people']-1;
					$data[$k]['recordinfo']	 	= unserialize($dish_set_recorddb['info']);
				}
    		}
    		
    	}
		$member_card_set_db=M('Member_card_set');
    	$thisCard=$member_card_set_db->where(array('token'=>$this->token,'id'=>intval($this->cardid)))->find();
		$this->assign('thisCard',$thisCard);
    	$this->assign('firstItemID',$data[0]['id']);
    	$this->assign('list',$data);
    	$this->assign('type',$type);
    ...
    //支付返回
	public function payCouponReturn(){
		$act = $_GET['act'];
		$cardid = $_GET['cardid'];
		...
		if($order){
			if($order['paid'] == 1){
				...
				if($order['type'] == 1){
					...
					if($order['type'] == 4){
						M('dish_set')->where(array('token'=>$this->token,'id'=>$itemid))->setInc('usetime');
						$data=array();
						$data['use_time'] 		= '';
						$data['add_time'] 		= time();
						$data['coupon_id'] 		= $itemid;
						...
						$Dish_setdb	= M('Dish_set')->where(array('token'=>$this->token,'id'=>$itemid))->find();
						$data['info'] 	= $Dish_setdb['info'];//var_dump($data);exit();
						$data['name'] 	= $Dish_setdb['name'];
						$data['image'] 	= $Dish_setdb['image'];
						
						//var_dump(M('dish_set_record')->getlastsql());exit();
						$list       = array();
						$cwhere = array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'coupon_id'=>$itemid);
						$dish_set_recorddb 	= M('dish_set_record')->where($cwhere)->find();
						if(!empty($dish_set_recorddb)){
							$taocaninfo = unserialize($dish_set_recorddb['info']);
							$setdbinfo = unserialize($Dish_setdb['info']);
							foreach ($taocaninfo as $row) {
								if(empty($row['name'])) continue;
								foreach ($setdbinfo as $row2) {
									if($row['dishid']==$row2['dishid']){
										$row['num']=$row['num']+$row2['num'];
									}
								}
								$list[]  = $row;
							}
							...
							if(empty($list2)){
								...
								$newdata['info']=serialize(array_merge($list,$list2));
							}
							
							$result=M('dish_set_record')->where($cwhere)->save($newdata);
						}else{
							$rid 	= M('dish_set_record')->add($data);
						}
				...
				if(empty($act)){
					$this->success('支付成功',U('Card/my_coupon',array('token'=>$token,'wecha_id'=>$wecha_id,'cardid'=>$cardid)));
				}else{
	...
	public function topayadmin(){
		...
		$member_card_create_db=M('Member_card_create');
		$thisCard=$member_card_set_db->where(array('token'=>$this->token,'id'=>intval($_GET['cardid'])))->find();
		$card = $member_card_create_db->field('number')->where(array('token'=>$info['token'],'wecha_id'=>$info['user_wecha_id']))->find();
		$company_model=M('Company');
		
		$cardsCount=$member_card_create_db->where(array('token'=>$this->token,'wecha_id'=>$info['user_wecha_id']))->count();
		$this->assign('cardsCount',$cardsCount);
		$token = $this->token;
		$thisCompany=$company_model->where("token = '$token'")->find();
		
		$row=array();
		$userinfolist = M('Userinfo')->where("token = '$token' AND wecha_id = '".$info['user_wecha_id']."'")->find();
		$carinfolist = M('usercarinfo')->where("token = '$token' AND wecha_id = '".$info['user_wecha_id']."'")->select();
		$row['truename'] =$userinfolist['truename'];
		$row['tel'] =$userinfolist['tel'];
		$row['carinfolist'] =$carinfolist;
		$ii=1;
		if(count($carinfolist)>1){
			foreach ($carinfolist as $carow) {
				if($ii>=count($carinfolist)){
					$row['carnumber'].=$carow['carnumber'];
				}else{
					$row['carnumber'].=$carow['carnumber'].'，';
				}
				$ii++;
			...
			$row['carnumber'] =$carinfolist[0]['carnumber'];
		}
	...
	public function consume(){
		...
		if(IS_POST){	
			...
			if($pay_type == 1){
					...
					$record['orderid'] 		= $single_orderid;
					$record['notes']		= $this->_post('notes','trim');
					$record['ordername'] 	= $this->_post('ordername','trim');
					$record['paytype'] 		= 'CardPay';
					$record['createtime'] 	= time();
					$record['paid'] 		= 0;
					$record['price'] 		= $arr['expense'];
					...
					$result = M('Member_card_pay_record')->add($record);	
					M('Member_card_coupon_record')->where($rwhere)->save(array('use_time'=>time(),'is_use'=>'1'));
					$this->redirect(U('CardPay/pay',array('from'=>'Card','token'=>$this->token,'wecha_id'=>$this->wecha_id,'price'=>$arr['expense'],'cardid'=>$cardid,'single_orderid'=>$single_orderid,'orderName'=>'支付除优惠劵外多余款项','redirect'=>'Card/payReturn|itemid:'.$itemid.',usecount:'.$arr['usecount'].',score:'.$arr['score'].',type:coupon,cardid:'.$cardid)));
					exit;
			}else if($pay_type == 3){
					$paypass 	= $this->fans['paypass'];		
					$arr['itemid']		= $itemid;
					...
					$record['orderid'] 		= $single_orderid;
					$record['notes']		= $this->_post('notes','trim');
					$record['ordername'] 	= empty($record['notes'])?'现金支付除优惠劵外的款项':$record['notes'];
					$record['paytype'] 		= 'CardPay';
					$record['createtime'] 	= time();
					$record['paid'] 		= 0;
					$record['price'] 		= $arr['expense'];
					...
					$this -> success('正在提交中...',U('CardPay/pay',array('from'=>'Card','token'=>$this->token,'wecha_id'=>$this->wecha_id,'price'=>$arr['expense'],'cardid'=>$cardid,'single_orderid'=>$single_orderid,'orderName'=>$record['ordername'])));
					exit;
...
public function consumeadmin(){
		...
		if(IS_POST){	
			$rwhere 	= array('token'=>$this->token,'wecha_id'=> $user_wecha_id,'coupon_type'=>array('lt','3'),'id'=>$this->_post('consume_id','intval'),'is_use'=>'0');
			$r_record 	= M('Member_card_coupon_record')->where($rwhere)->find();
			if (!$r_record){
				$r_record['coupon_id'] = 0;
			}
			...
			if($pay_type == 1){
					...
					$result = M('Member_card_pay_record')->add($record);	
					M('Member_card_coupon_record')->where($rwhere)->save(array('use_time'=>time(),'is_use'=>'1'));
					$this -> success('正在提交中...',U('CardPay/pay',array('from'=>'Card','token'=>$this->token,'wecha_id'=>$this->wecha_id,'user_wecha_id'=> $user_wecha_id,'price'=>$arr['expense'],'cardid'=>$cardid,'single_orderid'=>$single_orderid,'orderName'=>'管理员代客消费')));
					exit;
			}
		}else{
			$card = M('Member_card_create')->field('number')->where(array('token'=>$this->token,'wecha_id'=>$user_wecha_id))->find();
			$this->assign('card',$card);
			$this->assign('consume_info',$data);
			$this->assign('thisCard',$thisCard);
			$this->display();		
	...
	public function payAction(){
		...
		if($orderid != ''){
			...
			if($res){
				$this->success('提交成功，正在跳转支付页面..',U('Alipay/pay',array('from'=>'Card','orderName'=>$res['ordername'],'single_orderid'=>$res['orderid'],'cardid'=>$cardid,'token'=>$res['token'],'wecha_id'=>$res['wecha_id'],'price'=>$res['price'])));
			}else{
				$this->error('无此订单');
			}
		}
		...
		if($record->create($_POST)){
			if($record->add($_POST)){
				$this->success('提交成功，正在跳转到支付页面..',U('Alipay/pay',array('from'=>'Card','orderName'=>$_POST['orderName'],'single_orderid'=>$_POST['orderid'],'cardid'=>$cardid,'token'=>$_POST['token'],'wecha_id'=>$_POST['wecha_id'],'price'=>$price)));
			}
		}else{
			$this->error('系统错误');
		}
		...
		if($order){
			if($order['paid'] == 1){
				...
				if($order['type'] == 1){
					...
					$Userinfo = M('Userinfo')->where("wecha_id = '$wecha_id' AND token = '$token'")->limit(1)->find();
					
					$member_card_set_db=M('Member_card_set');
					$thisCard=$member_card_set_db->where(array('token'=>$this->token,'minimomey'=>array('elt',$Userinfo['balance'])))->order('minimomey desc')->limit(1)->find();
					
					$member_card_create_db=M('Member_card_create');
					$card = $member_card_create_db->field('number')->where("wecha_id = '$wecha_id' AND token = '$token'")->find();
					if($card){
						if($member_card_create_db->where("wecha_id = '$wecha_id' AND token = '$token'")->save(array('cardid'=>$thisCard['id']))){
							$cardid=$thisCard['id'];
						}
					}
				}else{
	...
	public function payReturnadmin(){
		...
		$this -> redirect(U('Card/payRecordadmin', array('token' => $this -> token,  'wecha_id' => $this -> wecha_id,'cid' => $this -> _cid)));
    }
	public function rechargeadmin(){
	  $db = M('Userinfo');
	  $uid = (int)$_GET['uid'];
	  $uinfo = $db->where(array('id'=>$uid))->field('wechaname,wecha_id,truename,id')->find();
	  if($this->isadmin['level']>0){
			  if(IS_POST){
				  ...
				  if($db->where(array('token'=>$this->token,'id'=>$id))->setInc('balance',$_POST['price'])){
					  ...
					  $myuinfo = $db->where(array('token'=>$this->token,'wecha_id'=>$this->wecha_id))->field('wechaname,wecha_id,truename,id')->find();
					  M('Member_card_pay_record')->add(array('orderid' => $orderid , 'ordername' => '微信管理后台手动充值' , 'createtime' => time() , 'token' => $this->token , 'wecha_id' => $uinfo['wecha_id'] , 'admin_wecha_id' =>$this->wecha_id , 'adminname' => $myuinfo['truename'] , 'price' => $_POST['price'] , 'type' => 1 , 'paid' => 1 , 'module' => 'Card' , 'paytime' => time() , 'paytype' => 'recharge'));
					  $uinfo = $db->where(array('token'=>$this->token,'id'=>$uid))->find();
					  $member_card_set_db=M('Member_card_set');
					  $thisCard=$member_card_set_db->where(array('token'=>$this->token,'minimomey'=>array('elt',$uinfo['balance'])))->order('minimomey desc')->limit(1)->find();
					  
					  $member_card_create_db=M('Member_card_create');
					  $card = $member_card_create_db->field('number')->where(array('token' => $this->token , 'wecha_id' => $uinfo['wecha_id']))->find();
					  if($card){
						  if($member_card_create_db->where(array('token' => $this->token , 'wecha_id' => $uinfo['wecha_id']))->save(array('cardid'=>$thisCard['id']))){
							  $cardid=$thisCard['id'];
						  }
					  }
					  $this->success('充值成功',U('Card/cardadmin',array('token'=>$this->token,'wecha_id'=>$this->wecha_id)));
				  }else{
			...
			$this->error('非管理员，禁止操作！',U('Card/card',array('token'=>$this->token,'wecha_id'=>$uinfo['wecha_id'])));
		}
		//$this->display();
	}
	public function payRecordadmin(){
		$token = $this->token;
		...
		if($this->isadmin['level']>0){
			...
			if($m != ''){
				...
				$last = strtotime(date('Y-m-d',$start)." +1 month -1 day");
				$count      = $record->where("token = '$token' AND paid=1  AND createtime < $last AND createtime > $start")->count();
				$Page       = new Page($count,15);
				$show       = $Page->show();
				$this->assign('page', $show);
				
				$list = $record->where("token = '$token' AND paid=1  AND createtime < $last AND createtime > $start")->limit($Page->firstRow.','.$Page->listRows)->order('createtime DESC')->select();
				//if(!$list) {$this->error("抱歉，无此用户记录！");exit;}
			}else if($user_wecha_id!=''){
				$count      = $record->where("token = '$token' AND paid=1 AND wecha_id = '$user_wecha_id' ")->count();
				$Page       = new Page($count,15);
				$show       = $Page->show();
				$this->assign('page', $show);
				$list = $record->where("token = '$token' AND paid=1 AND wecha_id = '$user_wecha_id' ")->limit($Page->firstRow.','.$Page->listRows)->order('createtime DESC')->select();
			}else{
				$count      = $record->where("token = '$token' AND paid=1  ")->count();
				$Page       = new Page($count,15);
				$show       = $Page->show();
				$this->assign('page', $show);
				$list = $record->where("token = '$token' AND paid=1  ")->limit($Page->firstRow.','.$Page->listRows)->order('createtime DESC')->select();
				//if(!$list) {$this->error("抱歉，无此用户记录！");exit;}
			}
			$recordlist       = array();
        	foreach ($list as $row) {
				$userinfolist = M('Userinfo')->where("token = '$token' AND wecha_id = '".$row['wecha_id']."'")->find();
				$dishorder = M('dish_order')->where("token = '$token' AND orderid = '".$row['orderid']."'")->find();
				$carinfolist = M('usercarinfo')->where("token = '$token' AND wecha_id = '".$row['wecha_id']."'")->select();
			    $row['truename'] =$userinfolist['truename'];
				$row['tel'] =$userinfolist['tel'];
				$row['carinfolist'] =$carinfolist;
				if($dishorder){
					$row['des'] =$dishorder['des'];
					$row['reservetime'] =$dishorder['reservetime'];
					$row['info'] = unserialize($dishorder['info']);
				   ...
				   $row['des'] =empty($row['des'])?$row['notes']:'';
				}
			
				if(empty($row['adminname'])&&empty($row['admin_wecha_id'])){
						$row['adminname']='';
						if($dishorder){
							$optype=2;//后台代客下单
						}
						...
						if(!$dishorder){
							if($row['paytype']=='recharge')
							{
								$optype=1;//后台充值
							}else if($row['paytype']=='CardPay')
							{
								$optype=3;//后台代客消费
							}
						}
						...
						}else if(!empty($row['admin_wecha_id'])){
							$row['adminname'] ='管理员：';
	...
	public function payRecord(){
		...
		if($m != ''){
			...
			$start = strtotime($nowY."-".$m."-01");
			$last = strtotime(date('Y-m-d',$start)." +1 month -1 day");
			$count      = $record->where("token = '$token' AND wecha_id = '$wecha_id' AND paid=1 AND createtime < $last AND createtime > $start")->count();
			$Page       = new Page($count,15);
			$show       = $Page->show();
			$this->assign('page', $show);
			$list = $record->where("token = '$token' AND wecha_id = '$wecha_id' AND paid=1 AND createtime < $last AND createtime > $start")->limit($Page->firstRow.','.$Page->listRows)->order('createtime DESC')->select();
			//if(!$list) {$this->error("抱歉，无此用户记录！");exit;}
		}else{
			$count      = $record->where("token = '$token' AND paid=1 AND wecha_id = '$wecha_id'")->count();
			$Page       = new Page($count,15);
			$show       = $Page->show();
			$this->assign('page', $show);
			$list = $record->where("token = '$token' AND paid=1 AND wecha_id = '$wecha_id'")->limit($Page->firstRow.','.$Page->listRows)->order('createtime DESC')->select();
			//if(!$list) {$this->error("抱歉，无此用户记录！");exit;}
		}
