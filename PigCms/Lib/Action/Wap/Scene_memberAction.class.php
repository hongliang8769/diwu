...
class Scene_memberAction extends WapAction{
	...
	public function index(){
		...
		if(!empty($mInfo) && $this->_get('name') == 'vote'){
			header("Location:".U('Scene_vote/index',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'id'=>$act_id,'act_type'=>$act_type),true,false,true));
		}
	...
	public function result($token,$act_name,$act_id,$act_type,$is_add){
		$this->token=$token;
		if($act_name == 'shake'){
			echo json_encode(array('err'=>0,'name'=>$act_name,'href'=>U('Shake/index',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'id'=>$act_id,'act_type'=>$act_type),true,false,true)));
			exit();
			...
			echo json_encode(array('err'=>0,'name'=>$act_name,'href'=>U('Scene_vote/index',array('token'=>$this->token,'wecha_id'=>$this->wecha_id,'id'=>$act_id,'act_type'=>$act_type),true,false,true)));
			exit();
		}else{
