...
//导入excel会员卡
	...
	public function localUploadCardExcel(){
		...
		if ($return['error']){
			...
			//
			$sheet=$data->sheets[0];
			$rows=$sheet['cells'];//var_dump($rows);exit();
			if ($rows){
				$i=0;
				foreach ($rows as $r){
					echo $i;
				//跳过表头
					if($i != 0){
						...
						//1卡号	2姓名	3手机号	4积分	5微信名	6性别	7出生年	8出生月	9出生日	10头像地址	11QQ号 12领卡时间  13消费总额 14余额  15 wecha_id
						//1姓名 2性别 3会员卡	4积分	5余额 6手机号	 7车型 8车牌号 9通讯地址 10会员备注 11开户日期  12 wecha_id
						//  cardid  token   
						if($r[2] == '男'){
							$r[2] = 1;
						}elseif($r[2] == '女'){
						...
						$userinfo = array('token'=>$this->token,'wecha_id'=>htmlspecialchars($r[12]),'truename'=>htmlspecialchars($r[1]),'sex'=>$r[2],'total_score'=>(int)$r[4],'balance'=>(float)$r[5],'tel'=>htmlspecialchars($r[6]),'address'=>htmlspecialchars($r[9]),'beizhu'=>htmlspecialchars($r[10]),'getcardtime'=>strtotime($r[11]));
						//$info2 = array('number'=>$r[1],'cardid'=>(int)$_POST['id'],'token'=>$this->token,'wecha_id'=>$r[15]);	
						$carinfo = array('carnumber'=>htmlspecialchars($r[8]),'cartype'=>htmlspecialchars($r[7]),'token'=>$this->token,'wecha_id'=>htmlspecialchars($r[12]));
						$carwhere=array('carnumber'=>htmlspecialchars($r[8]),'token'=>$this->token);//var_dump($carwhere);
						//if(preg_match("/^[\x{4e00}-\x{9fa5}]{1}[A-Za-z0-9]{6}$/u", $carinfo['carnumber'])){
						if(self::abslength($carinfo['carnumber'])>1){
							$userCarExist=M('Usercarinfo')->where($carwhere)->find();	
							if(!$userCarExist){
								M('Usercarinfo')->add($carinfo);
							}else{
								M('Usercarinfo')->where($carwhere)->save($carinfo);
							}
						}	
						$userwhere = array('tel'=>htmlspecialchars($r[6]),'token'=>$this->token);	
						if(preg_match_all("/^1[3|4|5|8]\d{9}$/", $userinfo['tel'], $match)){
							$db_exist = $db->where($userwhere)->field('id')->find();
							if(!$db_exist){
								$db->add($userinfo);
							}else{
								$db->where($userwhere)->save($userinfo);
								//M('Usercarinfo')->add(array('yanse'=>htmlspecialchars($r[1]),'token'=>'6'));
							}
						}else{
							$userwhere = array('truename'=>htmlspecialchars($r[1]),'token'=>$this->token);
							if(self::abslength($userinfo['truename'])>1){
								$db_exist = $db->where($userwhere)->field('id')->find();
								if(!$db_exist){
									$db->add($userinfo);
								}else{
									$userwhere = array('truename'=>htmlspecialchars($r[1]),'beizhu'=>htmlspecialchars($r[10]),'token'=>$this->token);
									if(self::abslength($userinfo['beizhu'])>1){
										$db_exist = $db->where($userwhere)->field('id')->find();
										if(!$db_exist){
											$db->add($userinfo);
										}else{
											$db->where($userwhere)->save($userinfo);
											//M('Usercarinfo')->add(array('yanse'=>htmlspecialchars($r[1]),'token'=>'7'));
										}
									}else{
										$db->where(array('truename'=>htmlspecialchars($r[1]),'token'=>$this->token))->save($userinfo);
										//M('Usercarinfo')->add(array('yanse'=>htmlspecialchars($r[1]),'token'=>'8'));
									}
								}
							}
						}	
						$userwhere = array('wecha_id'=>htmlspecialchars($r[12]),'token'=>$this->token);
						$db_exist = M('Userinfo')->where($userwhere)->field('id')->find();	
						$thisCardinfo=M('Member_card_set')->where(array('token'=>$this->token,'cardname'=>htmlspecialchars($r[3])))->find();
						if($thisCardinfo){
							$card=M('Member_card_create')->where(array('token'=>$this->token,'cardid'=>$thisCardinfo['id'],'wecha_id'=>htmlspecialchars($r[12])))->find();
							if(!$card){
								$cardcreat=M('Member_card_create')->field('id,number')->where(array('token'=>$this->token,'cardid'=>$thisCardinfo['id'],'wecha_id'=>''))->order('id ASC')->find();
								if($cardcreat){
									M('Member_card_create')->where(array('id'=>$cardcreat['id']))->save(array('userinfoid'=>$db_exist['id'],'wecha_id'=>htmlspecialchars($r[12])));
								}
							}else{
								M('Member_card_create')->where(array('token'=>$this->token,'cardid'=>$thisCardinfo['id'],'wecha_id'=>htmlspecialchars($r[12])))->save(array('userinfoid'=>$db_exist['id']));
							}
						}
