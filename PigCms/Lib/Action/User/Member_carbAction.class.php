<?php
class Member_carbAction extends UserAction{
	...
	public function recharge(){
		...
		if(IS_POST){
			...
			if($db->where(array('token'=>$this->token,'id'=>$id))->setInc('balance',$_POST['price'])){
				...
				M('Member_card_pay_record')->add(array('orderid' => $orderid , 'ordername' => '后台手动充值' , 'createtime' => time() , 'token' => $this->token , 'wecha_id' => $uinfo['wecha_id'] ,'admin_wecha_id' =>$Think.session.uid , 'adminname' => $Think.session.uname , 'price' => $_POST['price'] , 'type' => 1 , 'paid' => 1 , 'module' => 'Card' , 'paytime' => time() , 'paytype' => 'recharge'));
				$this->success('充值成功');
			}else{
