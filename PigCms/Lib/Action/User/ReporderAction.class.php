<?php
class ReporderAction extends BaseAction{
    public $_cid = 0;
...
//					$this->error('非法操作', U('Repast/index',array('token' => $this->token)));
	...
	public function orderlist() {
		...
		$this->assign('tpage', $company['radius']);	
		$this->assign('ttime', $company['money']*1000);
		$this->display();
	}
	public function getorderlist() {
		$token = $this->token;
		$pay_record=M('Dish_order');
		$where=array('token'=>$this->token,'paid'=>1);
		$company = M('Dish_company') -> where(array('cid' => $this->_cid )) -> find();
		
		$count      = $pay_record->where($where)->count();
		$Page       = new Page($count,$company['scope']);
		$show       = $Page->show();
		$list = $pay_record->where($where)->limit($Page->firstRow.','.$Page->listRows)->order('paytime DESC')->select();
		$finfolist       = array();
		foreach ($list as $row) {
			$userinfolist = M('Userinfo')->where(array('token'=>$this->token,'wecha_id'=>$row['wecha_id']))->find();
			$carinfolist = M('usercarinfo')->where(array('token'=>$this->token,'wecha_id'=>$row['wecha_id']))->select();
			$card = M('Member_card_create')->field('cardid')->where(array('token' => $this->token , 'wecha_id' => $row['wecha_id']))->find();
			$thisCard=M('Member_card_set')->where(array('token'=>$this->token,'id'=>$card['cardid']))->find();
			$row['cardname']=$thisCard['cardname'];
			$row['truename'] =$userinfolist['truename'];
			$row['tel'] =$userinfolist['tel'];
			$row['paytime'] =date('d日 H:i',$row['paytime']);
			$ii=1;
		...
		$count2      = $pay_record->where("token = '$token'   AND reservetime < $last AND reservetime > $start")->count();
		$Page2       = new Page($count2,$company['scope']);
		$show2       = $Page2->show();
		$list2 = $pay_record->where("token = '$token'   AND reservetime < $last AND reservetime > $start")->limit($Page2->firstRow.','.$Page2->listRows)->order('reservetime DESC')->select();
		$yinfolist       = array();
		foreach ($list2 as $row) {
			...
			$row['truename'] =$userinfolist['truename'];
			$row['tel'] =$userinfolist['tel'];
			$row['reservetime'] =date('d日 H:i',$row['reservetime']);
			$ii=1;
			if(count($carinfolist)>1){
				foreach ($carinfolist as $carow) {
					if($ii>=count($carinfolist)){
...
//		$this->assign('page', $show);	
//		$this->assign('list', $recordlist);
//		$this->assign('list2', $recordlist2);
		$result['yinfolist']=$yinfolist;
		$result['finfolist']=$finfolist;
