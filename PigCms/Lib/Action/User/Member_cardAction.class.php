...
class Member_cardAction extends UserAction{
	...
	public function useredit(){
		$uid=$this->_param('uid');
		$cardid=$this->_param('cardid');
		$itemid=$this->_param('itemid');
		$cid=$this->_param('cid');
		
		$userinfo=M('Userinfo')->where(array('id'=>$uid))->find();
		$userCarExist=M('Usercarinfo')->where(array('id'=>$cid))->find();
		...
		$cardinfo= M('Member_card_create')->where(array('id'=>$itemid))->find();

		$this->assign('info',$userinfo);
		$this->assign('carinfo',$userCarExist);
		$this->assign('cardInfo',$cardinfo);
		$this->assign('cardid',$cardinfo['cardid']);
		...
		if (IS_POST){
			...
			if($data['wecha_id'] == ''){
							...
							for($j=0;$j<28;$j++){
								$rand = mt_rand(0,61);
								$data['wecha_id'] .= $str[$rand];
							}
			}
			/*if(M('Member_card_custom')->where(array('token'=>$this->token))->getField('carnumber')){
				if(empty($data['carnumber'])){
					$this->error("车牌号号必填。");exit;
				}
			}*/
			$bcar=M('Member_card_custom')->where(array('token'=>$this->token))->getField('carnumber');
			 ...
			 $this->_post('total_score')? $data['total_score'] = $this->_post('total_score') : $data['total_score'] = '';
			 //$this->_post('balance')? $data['balance'] = $this->_post('balance') : $data['balance'] = '';
			 
			 $this->_post('carnumber')? $data2['carnumber'] = $this->_post('shortname').$this->_post('carnumber') : $data2['carnumber'] = '';
			 $this->_post('yanse')? $data2['yanse'] = $this->_post('yanse') : $data2['yanse'] = '';
			 $this->_post('cartype')? $data2['cartype'] = $this->_post('cartype').$data2['yanse'] : $data2['cartype'] = '';
			 $data2['token'] =$data['token'];
			 $data2['wecha_id'] =$data['wecha_id'];

			if($this->_post('paypass') != ''){
				$data['paypass'] = md5($this->_post('paypass'));
			}
			if($uid>0){
				M('Userinfo')->where(array('id'=>$uid))->save($data);
			}else{
				$userinfo=M('Userinfo')->where(array('token'=>$this->token,'tel'=>$data['tel']))->find();
				if($userinfo){
					$uid=$userinfo['id'];
					M('Userinfo')->where(array('id'=>$uid))->save($data);
				}else{
					$data['getcardtime']=time();
					$uid=M('Userinfo')->add($data);//var_dump(M('Userinfo')->getLastSql());exit();
				}
				
			}	
			
			if($cid>0){
				M('Usercarinfo')->where(array('id'=>$cid))->save($data2);
			}else{
				$userCarExist=M('Usercarinfo')->where(array('token'=>$this->token,'carnumber'=>$data2['carnumber']))->find();
				if($userCarExist){
					$cid=$userCarExist['id'];
					M('Usercarinfo')->where(array('id'=>$cid))->save($data2);
				}else{
					$cid=M('Usercarinfo')->add($data2);
				}
			}
			...
			if($cardinfo){
				if($cardinfo['cardid']!=$cardid){
					 M('Member_card_create')->where(array('id'=>$itemid))->save(array('wecha_id'=>''));
					 $card=M('Member_card_create')->field('id,number')->where("token='".$this->token."' and cardid=".$cardid." and wecha_id = ''")->order('id ASC')->find();
					 if($card){
						$card_up=M('Member_card_create')->where(array('id'=>$card['id']))->save(array('wecha_id'=>$data['wecha_id']));
					  }else{
						  $this->error("该商家会员卡缺货了。");exit;
					  }
				}
			}else{
				if($cardid>0){
					...
					if($card){
						$card_up=M('Member_card_create')->where(array('id'=>$card['id']))->save(array('wecha_id'=>$data['wecha_id']));
						//var_dump(M('Member_card_create')->getLastSql());exit();
					}else{
						$this->error("该商家会员卡缺货了。");exit;
					}
				}
			...
			$this->success('操作成功',U('Member_card/center',array('token'=>$this->token)));
		}else {
			$this->display();
		}
	}
	...
	public function create(){
		...
		if (IS_POST){//删除操作
			...
			$show       = $Page->show();
			$list = $data->where($where)->order('number asc')->limit($Page->firstRow.','.$Page->listRows)->select();
			//计算领取张数
	...
	*/
	...
	public function staffSet(){
		$company_staff_db=M('Company_staff');
		if(IS_POST){
			if (!trim($_POST['username'])||!intval($_POST['companyid'])){
				$this->error('用户名和店铺都不能为空');
				exit();
			}
			$_POST['token']=$this->token;
			$_POST['time']=time();
	...
	//解除指定会员绑定，在会员管理中操作
	public function member_delbang(){
		$card_create_db=M('Member_card_create');
	...
	public function member_del(){
		...
		M('Member_card_use_record')->where($where)->delete();//消费记录
		M('member_card_pay_record')->where($where)->delete();
		M('usercarinfo')->where($where)->delete();
		M('Userinfo')->where($where)->delete();
		$card_create_db->where(array('id'=>intval($_GET['itemid'])))->save(array('wecha_id'=>''));
		$this->success('操作成功');
	}
	public function member_del2(){
		$uid=$this->_param('uid');
		$thisUser=M('Userinfo')->where(array('id'=>$uid))->find();
		if($thisUser){
			$where=array('wecha_id'=>$thisUser['wecha_id'],'token'=>$this->token);
			M('Member_card_sign')->where($where)->delete();
			M('Member_card_use_record')->where($where)->delete();//消费记录
			...
			M('usercarinfo')->where($where)->delete();
			M('Userinfo')->where($where)->delete();
			M('Member_card_create')->where($where)->save(array('wecha_id'=>''));
		}
		$this->success('操作成功');
	}
	//解除全部会员卡下的全部会员绑定
	public function deleteallbang(){
		$tokenWhere=array('token'=>$this->token);
		M('Member_card_create')->where($tokenWhere)->save(array('wecha_id'=>''));
		$this->success('操作成功');
	}
	//删除全部非绑定会员
	public function deletealluser(){
		$tokenWhere=array('token'=>$this->token);
		M('Member_card_sign')->where($tokenWhere)->delete();
		M('Member_card_use_record')->where($tokenWhere)->delete();//消费记录
		M('member_card_pay_record')->where($tokenWhere)->delete();
		M('usercarinfo')->where($tokenWhere)->delete();
		M('Userinfo')->where($tokenWhere)->delete();
		$this->success('操作成功');
	}
	//解除指定会员卡下的全部会员绑定
	public function deletecardbang(){
		$tokenWhere=array('token'=>$this->token,'cardid'=>$_GET['id']);
		M('Member_card_create')->where($tokenWhere)->save(array('wecha_id'=>''));
		$this->success('操作成功');
	}
	//删除指定会员卡下的全部绑定会员
	public function deletecardmember(){
		$tokenWhere=array('token'=>$this->token,'cardid'=>$_GET['id']);
		$allcard=M('Member_card_create')->where($tokenWhere)->select();
		if ($allcard){
			//$this->error('请先删除创建的卡号');
			foreach($allcard as $key=>$value){
				if ($value['wecha_id']){
					$where=array('wecha_id'=>$value['wecha_id'],'token'=>$this->token);
					M('Member_card_sign')->where($where)->delete();
					M('Member_card_use_record')->where($where)->delete();//消费记录
					M('member_card_pay_record')->where($where)->delete();
					M('usercarinfo')->where($where)->delete();
	...
	public function delete(){
		...
		M('Member_card_create')->where($tokenWhere)->delete();
...
//导出会员卡
	...
	public function exportCard(){
		...
		$id = $this->_get('id','intval');
		$token = $this->token;
		//1姓名 2性别 3会员卡	4积分	5余额 6手机号	 7车型 8车牌号 9通讯地址 10会员备注  11 开户日期
		$arr = array(
			
			array('en'=>'truename','cn'=>'姓名'),
			array('en'=>'sex','cn'=>'性别 ( 男； 女； 其他）'),
			array('en'=>'cardid','cn'=>'会员卡'),
			//array('en'=>'cardtype','cn'=>'卡类型'),
			array('en'=>'total_score','cn'=>'积分'),
			array('en'=>'balance','cn'=>'余额'),
			array('en'=>'tel','cn'=>'手机号'),
			array('en'=>'cartype','cn'=>'车型'),
			array('en'=>'carnumber','cn'=>'车牌号'),
			array('en'=>'address','cn'=>'通讯地址'),
			array('en'=>'beizhu','cn'=>'会员备注'),
			array('en'=>'getcardtime','cn'=>'开户日期'),
		...
		$info = M('Userinfo')->join('tp_Usercarinfo on tp_Usercarinfo.wecha_id=tp_Userinfo.wecha_id')->join('tp_Member_card_create on tp_Member_card_create.wecha_id=tp_Userinfo.wecha_id')->field('tp_Userinfo.truename,tp_Userinfo.sex,tp_Userinfo.tel,tp_Userinfo.total_score,tp_Userinfo.balance,tp_Userinfo.address,tp_Userinfo.beizhu,tp_Userinfo.getcardtime,tp_Userinfo.wecha_id,tp_Usercarinfo.carnumber,tp_Usercarinfo.cartype,tp_Member_card_create.cardid')->where($where)->order('tp_Userinfo.getcardtime desc')->select();
		...
		if($info){
				foreach($info as $key=>$val){
					...
					foreach ($arr as $field){
						$fieldValue = str_replace(PHP_EOL, '', $val[$field['en']]);
						
						switch($field['en']){
						
							/*case 'cardtype':
								$thisCardinfo=M('Member_card_set')->where(array('token'=>$this->token,'id'=>$val['cardid']))->find();
								$thisCardinfo?$fieldValue = $thisCardinfo['cardname']:'非会员';	
								$fieldValue = iconv('utf-8','gbk',$fieldValue);
								break;*/
							case 'cardid':
								$thisCardinfo=M('Member_card_set')->where(array('token'=>$this->token,'id'=>$fieldValue))->find();
								$thisCardinfo?$fieldValue = $thisCardinfo['cardname']:'非会员';	
								$fieldValue = iconv('utf-8','gbk',$fieldValue);
								break;
							case 'truename':
...
//删除记录
	...
	public function centertao(){
		$uid=$this->_param('uid');
		$userinfo=M('Userinfo')->where(array('id'=>$uid))->find();
		$cwhere = array('token'=>$this->token,'wecha_id'=>$this->_param('wecha_id'));
	...
	if(!empty($search)){
			$Page->parameter   .=   'token='.$this->token.'&search='.$search;
	}		
//			$show       = $Page->show();
