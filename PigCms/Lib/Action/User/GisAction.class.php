...
class GisAction extends Action{
	...
	//添加公众帐号
	public function add(){
		$car=$this->_get('car',"htmlspecialchars");
		$cardata=explode(",",$car);
/*		$where['mcode']=session('uid');
		$where['id']=$id;
		$res=M('Wxuser')->where($where)->find();*/
		
		//$re=M('gisdata')->where(array('mcode'=>$cardata[0]))->save(array('lng'=>trim($cardata[1]),'lat'=>trim($cardata[2])));
		$re=M('gisdata')->add(array('lng'=>trim($cardata[1]),'lat'=>trim($cardata[2])));
		return $re;
	...
	public function upsave(){
		...
		if($db->create()===false){
			...
			if($id){
				if (isset($_POST['demoStep'])){
					header('Location:'.U('Index/bindHelp',array('id'=>intval($_POST['id'])),true,false,true));
				}else {
